...
def soundex_word(word, size):
    ...
    if first_letter in consonants and consonants[first_letter] == adjacent_free[
        0]:
        return first_letter.upper() + ''.join(
            str(char) for char in adjacent_free[1:size])

    return first_letter.upper() + ''.join(
        str(char) for char in adjacent_free[:size])
